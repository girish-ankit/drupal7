...
function stability_preprocess_html(&$variables) {
  ...
  // Add required skin
  drupal_add_css(drupal_get_path('theme', 'stability') . '/css/skins/' . theme_get_setting('skin') . '.css', array('group' => CSS_THEME));
  // Layout setting
  if(theme_get_setting('layout') == 'boxed') {
...
function stability_process_page(&$variables) {
  ...
  $header_top_menu_tree = module_exists('i18n') ? i18n_menu_translated_tree('menu-header-top-menu') : menu_tree('menu-header-top-menu');
  ...
  drupal_add_js(array('stability' => array('flickr_id' => theme_get_setting('flickr_id'))), 'setting');
}
